...
def custom_summary(mydf):
    ...
    for i in df.columns:
        if(mydf[i].dtype != object):
            cols.append(i)
    result = pd.DataFrame(columns=cols,index=['Datatype','Count','Min','Q1','Q2','Q3','Max','Mean','Std_dev','Skew','Kurt','Range','IQR','Skew_comment','Kurt_comment','Outlier_comment'])
...
def cv_post_hpt(x,y,fold = 10):
    ...
    score_dt = cross_val_score(DecisionTreeRegressor(criterion= 'friedman_mse' ,max_depth= 11 ,max_features= 3),x,y,cv= fold)
    score_kn = cross_val_score(KNeighborsRegressor(weights ='distance' ,algorithm ='brute' ),x,y,cv=fold)
    score_rf = cross_val_score(RandomForestRegressor(max_depth= 11,max_features='auto' ,n_estimators= 150),x,y,cv=fold)
...
model_names = [LinearRegression(),Lasso(),Ridge(),SVR(),DecisionTreeRegressor(),KNeighborsRegressor(),RandomForestRegressor(),AdaBoostRegressor(),GradientBoostingRegressor(),XGBRegressor()]
